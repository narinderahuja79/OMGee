...
?>
	<tr>
		...
		<td class="order-id">
			...
			<?php
					if(isset($dev['vendor'])){
						echo /*$this->crud_model->get_type_name_by_id('vendor', $dev['vendor'], 'username').' ('.translate('vendor').') : '.*/ucwords($dev['status']);
					} else if(isset($dev['admin'])) {
						echo /*translate('admin').' : '.*/ucwords($dev['status']);
					}
			?>
			</span>
			<br>
		...
		<td class="order-id">
			...
			<?php
					if(isset($dev['vendor'])){
						echo /*$this->crud_model->get_type_name_by_id('vendor', $dev['vendor'], 'name').' ('.translate('vendor').') : '.*/ucwords($dev['status']);
					} else if(isset($dev['admin'])) {
						echo /*translate('admin').' : '.*/ucwords($dev['status']);
					}
			?>
