		...
		<table id="demo-table" class="table table-striped"  data-pagination="true" data-show-refresh="true" data-ignorecol="0,4" data-show-toggle="true" data-show-columns="false" data-search="true" >
			...
			?>
                <tr>
                    ...
                    <td>
                    <?php 
                    	$already_add_product_ar = json_decode($row['spec']);
                    	foreach(json_decode($already_add_product_ar->set) as $key => $productids) 
			            {
			                $products_name_arr[] = $this->db->get_where('product',array('product_id'=>$productids))->row()->title;
			            }
			            echo implode(",",$products_name_arr);
			            unset($products_name_arr);
                    ?>
                    </td>
                    <td>
                    	<?php
